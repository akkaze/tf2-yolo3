...
def DarknetTiny(name=None):
    ...
    x = MaxPool2D(2, 2, 'same', name="{0}_maxpooling_0".format(name))(x)
    x = DarknetConv(x, 32, 3)
    x = MaxPool2D(2, 2, 'same', name="{0}_maxpooling_1".format(name))(x)
    x = DarknetConv(x, 64, 3)
    x = MaxPool2D(2, 2, 'same', name="{0}_maxpooling_2".format(name))(x)
    x = DarknetConv(x, 128, 3)
    x = MaxPool2D(2, 2, 'same', name="{0}_maxpooling_3".format(name))(x)
    x = x_8 = DarknetConv(x, 256, 3)  # skip connection
    x = MaxPool2D(2, 2, 'same', name="{0}_maxpooling_4".format(name))(x)
    x = DarknetConv(x, 512, 3)
    x = MaxPool2D(2, 1, 'same', name="{0}_maxpooling_5".format(name))(x)
...
def YoloOutput(filters, anchors, classes, name=None):
    def yolo_output(x_in):
        x = inputs = Input(x_in.shape[1:])
        x = DarknetConv(x, filters * 2, 3)
        #x = tf.keras.backend.print_tensor(x)
        x = DarknetConv(x, anchors * (classes + 5), 1, batch_norm=True)
...
def yolo_boxes(pred, anchors, classes):
    ...
    # objectness = tf.keras.backend.print_tensor(objectness)
    # class_probs = tf.keras.backend.print_tensor(class_probs)
...
def yolo_nms(outputs, anchors, masks, classes, iou_threshold=0.8, score_threshold=0.001):
    # boxes, conf, type
    b, c, t = [], [], []
